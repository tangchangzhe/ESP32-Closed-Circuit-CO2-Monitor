...
**问题**：密封气路不可避免会随时间泄漏，导致 CO2 读数出现系统性下降趋势（漂移近似指数衰减），掩盖真实的生物活动信号。
...
1. **锚点选择（周期法）**：选取一个或多个光暗周期的起止点作为锚点。由于漂移速率本身无法直接观测，而一个完整周期内既包含上升段（黑暗期）也包含下降段（光照期），对整个周期计算的斜率可以近似为该时段漂移速率的"中值"——既能将黑暗期被压低的曲线抬升回正常的上升趋势，又不会过度补偿导致光照期的下降曲线错误地向上
...
2. 选择时间范围查询数据
3. 在"漂移补偿"面板添加锚点时段
   - 每个锚点选择完整的**一个**光暗周期的起止点（锚点时段内包含的周期数越少，估算的漂移速率越精确）
